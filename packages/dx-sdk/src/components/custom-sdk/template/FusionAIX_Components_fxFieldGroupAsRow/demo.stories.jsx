...
import FusionAixComponentsFxFieldGroupAsRow from './index';

export default {
  title: 'Templates/Field Group As Row02',
  argTypes: {
    getPConnect: {
      table: {
        disable: true
  ...
  component: FusionAixComponentsFxFieldGroupAsRow
};
...
export const Default = {
  render: args => {
    ...
    const regionAChildren = mainResponse.children[0].children.map((child) => {
      return props.getPConnect().createComponent(child);
    });

    return (
      <FusionAixComponentsFxFieldGroupAsRow {...props}>{regionAChildren}</FusionAixComponentsFxFieldGroupAsRow>
    );
  },
  args: {
    heading: 'Heading'
