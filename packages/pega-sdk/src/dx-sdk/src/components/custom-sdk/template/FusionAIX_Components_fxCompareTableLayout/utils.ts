...
interface Field {
  type: string;
  children?: Field[];
  config: any; // You might want to replace 'any' with a more precise type if possible
}
...
export default function getAllFields(pConnect: any) {
  ...
  if (hasRegions) {
    metadata.children.forEach((region: any) =>
      region.children.forEach((field: Field) => {
        allFields.push(makeField(field));
        if (field.type === 'Group' && field.children) {
          field.children.forEach((gf: Field) => allFields.push(makeField(gf)));
        }
      })
    );
  } else {
